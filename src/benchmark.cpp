...
void printResult(const BenchmarkResult& result) {
    ...
    std::cout << " | Avg: " << std::setw(8) << std::right << result.averageTimeMs << " ms";
    std::cout << " | Min: " << std::setw(8) << result.minTimeMs << " ms";
    std::cout << " | Max: " << std::setw(8) << result.maxTimeMs << " ms";
    std::cout << " | StdDev: " << std::setw(6) << result.stdDevMs << " ms";
...
void compareKNNImplementations(const std::vector<Point>& trainData,
    ...
    std::cout << "\n=== KNN Implementation Comparison ===" << std::endl;
    std::cout << "Training size: " << trainData.size() << ", Test size: " << testData.size()
              << ", k: " << k << std::endl;
    std::cout << std::endl;
    ...
    auto standardResult = measure(
        "Standard KNN", [&]() { standardKNN.predictBatch(testData, k); }, 10, trainData.size());
    printResult(standardResult);

    // Optimized KNN with KD-tree
    KNNOptimized optimizedKNN(trainData, true);
    auto optimizedResult = measure(
        "Optimized KNN (KD-tree)", [&]() { optimizedKNN.predictBatch(testData, k); }, 10,
        trainData.size());
    printResult(optimizedResult);

    // Speedup
    double speedup = standardResult.averageTimeMs / optimizedResult.averageTimeMs;
...
void benchmarkKDTree(const std::vector<Point>& data, int k, size_t numQueries) {
    ...
    std::cout << "Data size: " << data.size() << ", Queries: " << numQueries << ", k: " << k
              << std::endl;
    std::cout << std::endl;
    ...
    // Linear search (standard KNN)
    KNN linearKNN(data);
    auto linearResult = measure(
        "Linear Search",
        [&]() {
            for (const auto& q : queries) {
                linearKNN.predict(q, k);
            }
        },
        10, data.size());
    ...
    auto kdTreeResult = measure(
        ...
        [&]() {
            for (const auto& q : queries) {
                kdTreeKNN.predict(q, k);
            }
        },
        10, data.size());
    printResult(kdTreeResult);

    double speedup = linearResult.averageTimeMs / kdTreeResult.averageTimeMs;
    std::cout << "\nSpeedup: " << std::fixed << std::setprecision(2) << speedup << "x"
              << std::endl;
}
...
void benchmarkParallelization(const std::vector<Point>& trainData,
                               const std::vector<Point>& testData, int k) {
    std::cout << "\n=== Parallel vs Sequential Prediction ===" << std::endl;
    std::cout << "Training size: " << trainData.size() << ", Test size: " << testData.size()
              << std::endl;
    std::cout << std::endl;
    ...
    // Sequential
    auto seqResult = measure("Sequential", [&]() { classifier.predictBatch(testData, k); }, 10,
                            testData.size());
    ...
    for (int threads : threadCounts) {
        std::string name = "Parallel (" + std::to_string(threads) + " threads)";
        auto parResult = measure(
            name, [&]() { classifier.predictBatchParallel(testData, k, threads); }, 10,
            testData.size());
        printResult(parResult);

        double speedup = seqResult.averageTimeMs / parResult.averageTimeMs;
...
void benchmarkWeightStrategies(const std::vector<Point>& trainData,
                                const std::vector<Point>& testData, int k) {
    std::cout << "\n=== Weight Strategy Comparison ===" << std::endl;
    std::cout << "Training size: " << trainData.size() << ", Test size: " << testData.size()
              << std::endl;
    std::cout << std::endl;
    ...
    for (const auto& strat : strategies) {
        ...
        auto result =
            measure(strat.name, [&]() { classifier.predictBatch(testData, k); }, 10, trainData.size());
        printResult(result);
...
void runFullSuite(int datasetSize, int testSize) {
    ...
    for (int size : sizes) {
        ...
        auto standardTime = measure(
            "Standard (size=" + std::to_string(size) + ")", [&]() { standard.predict(query, 5); },
            100, size);

        auto optimizedTime = measure(
            "KD-tree (size=" + std::to_string(size) + ")", [&]() { optimized.predict(query, 5); },
            100, size);

        printResult(standardTime);
        printResult(optimizedTime);

        double speedup = standardTime.averageTimeMs / optimizedTime.averageTimeMs;
        std::cout << "  → Speedup: " << std::fixed << std::setprecision(2) << speedup << "x\n"
                  << std::endl;
    }

    // Run detailed benchmarks on default dataset
    auto fullData = Dataset::generateSimpleDataset(datasetSize / 2, 42);
    auto [train, test] = Dataset::trainTestSplit(fullData, 0.8, true, 42);
...
bool saveResultsToCSV(const std::vector<BenchmarkResult>& results, const std::string& filename) {
    std::ofstream file(filename);
    if (!file.is_open()) {
        return false;
    }

    file << "Name,AverageTimeMs,MinTimeMs,MaxTimeMs,StdDevMs,Iterations,DataSize\n";

    for (const auto& result : results) {
        file << result.name << "," << result.averageTimeMs << "," << result.minTimeMs << ","
             << result.maxTimeMs << "," << result.stdDevMs << "," << result.iterations << ","
             << result.dataSize << "\n";
    }
